...
$subject = "fukuenyasan.jpへのお問い合わせ";
...
送信は正常に完了しました。
お急ぎかとは思いますが、ご相談内容確認後に内容に応じて適切な復縁専門カウンセラーを選任しいたしますので、しばらくお待ちください。
改めて担当者よりご連絡させていただきますのでお待ちください。

復縁屋さんではプランニング及び費用に関してのご相談も柔軟に対応しております。
個人情報の管理は最新のセキュリティ下で厳重に管理しておりますのでご安心ください。
...
TEL：03-6685-2008
E-mail:contact@fukuenyasan.jp
URL: https://fukuenyasan.jp/
──────────────────────
...
//配列の形「name="○○[]"」の場合には必ず後ろの[]を取ったものを指定して下さい。
$hankaku_array = array('ご連絡先電話番号','金額','Email','確認用Email');
...
if(($confirmDsp == 0 || $sendmail == 1) && $empty_flag != 1){
	...
	/**
	$bodycw = 'POS:1';
	$bodycw .= 'GET:' . print_r($_GET,1) . "\n";
	$bodycw .= 'POST:' . print_r($_POST,1) . "\n";
	$bodycw .= 'COOKIE:' . print_r($_COOKIE,1) . "\n";
	$bodycw .= 'confirmDsp:' . $confirmDsp . "\n";
	$bodycw .= 'sendmail:' . $sendmail . "\n";
	$bodycw .= 'empty_flag:' . $empty_flag . "\n";
	avm_chatwork_alert_room('TEST',$bodycw);
	**/
...
<title>入力内容の確認 | 復縁屋さん</title>
<style>
*,*::before,*::after{box-sizing:border-box}
body{font-family:'Hiragino Kaku Gothic ProN','Hiragino Sans',Meiryo,sans-serif;background:#FBF5F5;color:#333;font-size:15px;line-height:1.7;margin:0}
.page-wrap{max-width:680px;margin:0 auto;padding:32px 16px 60px}
.confirm-header{text-align:center;margin-bottom:28px}
.confirm-header h1{font-size:22px;font-weight:bold;margin-bottom:8px}
.confirm-header p{font-size:14px;color:#666}
.section-block{background:#fff;border-radius:8px;box-shadow:0 2px 8px rgba(0,0,0,.07);padding:20px;margin-bottom:16px}
.section-title{font-size:15px;font-weight:bold;color:#D13543;border-left:4px solid #D13543;padding-left:10px;margin-bottom:16px}
.confirm-row{display:flex;border-bottom:1px solid #f0f0f0;padding:10px 0;gap:12px}
.confirm-row:last-child{border-bottom:none}
.confirm-key{flex:0 0 38%;font-size:14px;color:#666;font-weight:bold}
.confirm-val{flex:1;font-size:15px;word-break:break-all}
p.error_messe{margin:5px 0;color:#D13543;font-size:14px}
.btn-area{display:flex;gap:12px;margin-top:24px;align-items:stretch}
.btn-back{flex:0 0 120px;padding:14px 10px;background:#f4f4f4;border:2px solid #ccc;border-radius:6px;font-size:15px;font-weight:bold;color:#555;cursor:pointer;text-align:center}
.btn-submit{flex:1;padding:16px 10px;background:linear-gradient(135deg,#E7B1BB 0%,#D13543 60%);color:#fff;font-size:18px;font-weight:bold;border:none;border-radius:6px;cursor:pointer;box-shadow:0 4px 12px rgba(209,53,67,.35);letter-spacing:.05em}
.trust-copy{font-size:13px;color:#888;text-align:center;margin-bottom:12px;line-height:1.7}
@media(max-width:480px){
.confirm-row{flex-direction:column;gap:4px}
.confirm-key{flex:none}
.btn-back{flex:0 0 100px;font-size:14px}
...
<p style="color:#D13543;font-weight:bold">入力にエラーがあります。「戻る」ボタンで修正してください。</p>
<?php echo $errm; ?>
<button type="button" onclick="history.back()" style="margin-top:16px;padding:10px 24px;background:#f4f4f4;border:2px solid #ccc;border-radius:4px;cursor:pointer;font-size:15px">← 戻る</button>
</div>
<?php }else{ ?>

<div class="confirm-header">
<h1>入力内容の確認</h1>
<p>以下の内容でよろしければ「申し込む」ボタンを押してください。<br>修正する場合は「戻る」ボタンをご利用ください。</p>
</div>
...
<button type="submit" class="btn-submit">この内容で申し込む</button>
</div>
</form>
...
function confirmOutput($arr){
	...
	// セクション定義：キー名 → セクションラベルのマップ
	$sections = array(
		'お客様情報'     => array('お名前','ご年齢','性別','お住まいの都道府県'),
		'現在のご状況'   => array('ご相談の種類','ご相談の種類（その他）','離別してしまった時期','現在お相手とはどのような状況','現在の状況（その他）','ご予算'),
		'ご相談内容'     => array('ご相談内容の詳細'),
		'ご連絡先'       => array('Email','確認用Email','ご連絡先電話番号'),
		'ご連絡希望日時' => array('連絡可能な日時','日付1','希望時間1','日付2','希望時間2'),
	);
	// スキップするシステムキー
	$skipKeys = array('mail_set','httpReferer','mailform_token','confirm_submit');
	...
	foreach($arr as $key => $val) {
		...
		if(is_array($val)){
			foreach($val as $key02 => $item){
				if(is_array($item)){ $out .= connect2val($item); }
				else { $out .= $item . '、'; }
			}
			$out = rtrim($out,'、');
		}else{ $out = $val; }
		...
		if($hankaku == 1){ $out = zenkaku2hankaku($key,$out,$hankaku_array); }
		if(trim($out) === '') continue; // 空行は非表示
		$processed[$key] = $out;
	}
	...
	$html = '';
	$usedKeys = array();
	foreach($sections as $sectionLabel => $sectionKeys) {
		$sectionRows = '';
		foreach($sectionKeys as $sk) {
			// 配列フィールド（[]なし）でのマッチング
			foreach($processed as $pkey => $pval) {
				$baseKey = preg_replace('/\[\]$/','',$pkey);
				if($baseKey === $sk && !in_array($pkey, $usedKeys)) {
					$usedKeys[] = $pkey;
					$dispOut = nl2br(h($pval));
					$dispOut = str_replace($replaceStr['before'], $replaceStr['after'], $dispOut);
					$hkey = h($pkey);
					$sectionRows .= '<div class="confirm-row">';
					$sectionRows .= '<div class="confirm-key">'.$hkey.'</div>';
					$sectionRows .= '<div class="confirm-val">'.$dispOut;
					$sectionRows .= '<input type="hidden" name="'.$hkey.'" value="'.str_replace(array("<br />","<br>"),"",$dispOut).'" />';
					$sectionRows .= '</div></div>'."\n";
				}
			}
		}
		if($sectionRows !== '') {
			...
			$html .= '<div class="section-title">'.h($sectionLabel).'</div>';
			$html .= $sectionRows;
			$html .= '</div>'."\n";
		}
	}
	...
	$remaining = '';
	foreach($processed as $pkey => $pval) {
		if(!in_array($pkey, $usedKeys)) {
			$dispOut = nl2br(h($pval));
			$dispOut = str_replace($replaceStr['before'], $replaceStr['after'], $dispOut);
			$hkey = h($pkey);
			$remaining .= '<div class="confirm-row">';
			$remaining .= '<div class="confirm-key">'.$hkey.'</div>';
			$remaining .= '<div class="confirm-val">'.$dispOut;
			$remaining .= '<input type="hidden" name="'.$hkey.'" value="'.str_replace(array("<br />","<br>"),"",$dispOut).'" />';
			$remaining .= '</div></div>'."\n";
		}
	}
	if($remaining !== '') {
		$html .= '<div class="section-block">'.$remaining.'</div>'."\n";
	}
...
function mailToAdmin($arr,$subject,$mailFooterDsp,$mailSignature,$encode,$confirmDsp){
	...
	avm_chatwork_alert_room($subject,$adminBody);
